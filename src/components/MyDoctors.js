...
class MyDoctors extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState({
      selectedList: nextProps.myDoctors[0],
      myDoctorsFromState: nextProps.myDoctors,
    })
  }
  render() {
    const { isLoader, isError, errorMessage, myDoctors,currentUser } = this.props
    const { selectedList,myDoctorsFromState } = this.state
    console.log(myDoctors, "selectedList",currentUser)
    return (
      <div style={{ backgroundColor: "#fff", }}>
        ...
        <div style={{ display: "flex", flexBasis: "100%", flexWrap: "wrap", justifyContent: "center", marginTop: "3%", }}>
          <div style={{ flexBasis: "40%", justifyContent: "center", display: "flex", borderRight: "2px solid", borderRightColor: "#F4F6FA", }}>
            <LargeList heading="Doctors name" data={myDoctors} func={(selectedList) => this.setState({ selectedList })} />
          ...
          <div style={{ flexBasis: "60%", }}>
            <div style={{ flexBasis: "100%", flexDirection: "row", display: "flex", padding: "1%", flexWrap: "wrap", borderBottom: "2px solid", borderBottomColor: "#F4F6FA", }}>
              <div style={{ width: 200, flexBasis: "30%", justifyContent: "center", display: "flex", alignItems: "center", fontWeight: "bold" }}>
                <img width="200" src={selectedList && selectedList.doctorImage} />
              </div>
              <div style={{ padding: "1%", flexBasis: "70%", }}>
                ...
                <div style={{ padding: "1%", alignItems: "center", display: "flex", color: "#C2BDBD", fontSize: 14 }}>
                  {selectedList && selectedList.fullName}

                </div>
                <div style={{ padding: "1%", alignItems: "center", display: "flex", color: "#C2BDBD", fontSize: 14 }}>
                  {selectedList && selectedList.otherName}

                </div>
                <div style={{ padding: "1%", alignItems: "center", display: "flex", color: "#C2BDBD", fontSize: 14 }}>
                  {selectedList && selectedList.MobileNumber}

                </div>
                <div style={{ padding: "1%", alignItems: "center", display: "flex", color: "#C2BDBD", fontSize: 14 }}>
                  {selectedList && selectedList.email}

                </div>
                <div style={{ padding: "1%", alignItems: "center", display: "flex", color: "#C2BDBD", fontSize: 14 }}>
                  {selectedList && selectedList.bussinesAddress}
                </div>
                <div style={{ padding: "1%", alignItems: "center", display: "flex", color: "#C2BDBD", fontSize: 14 }}>
                  {selectedList && selectedList.specialist}
                </div>
            ...
            <div style={{ flexBasis: "100%", flexDirection: "row", display: "flex", padding: "1%", flexWrap: "wrap", }}>
              <div style={{ width: 200, flexBasis: "30%", justifyContent: "center", display: "flex", alignItems: "center", fontWeight: "bold" }}>
     {selectedList&& <QRCode value={selectedList.DoctorId} />
     }         
                </div>
              <div style={{ padding: "1%", flexBasis: "70%", }}>
